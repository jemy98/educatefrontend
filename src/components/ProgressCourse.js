          ...
          {data.map((item, index) => (
            <div class="p-2 bg-indigo-600 h-24 w-3/4 rounded-2xl shadow-lg shadow-blue-500">
              <div className="mt-2 mb-2 text-white">
                ...
                <p className="float-right mr-4"> {item.progressmodul+item.progressquiz}/{() => getTotal(item.courseid._id)}</p>
              </div>
              <ProgressBar percentage={(item.progressmodul+item.progressquiz)/100} />
            </div>
